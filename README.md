...
Локальный MVP для загрузки CSV-каталога цен с готовыми embeddings, хранения каталога в PostgreSQL/pgvector, просмотра поставщиков во вкладке `Каталог`, сбора брифа и семантического поиска в ARGUS-style чате.
...
- Node.js 20+
- Docker для локального PostgreSQL с расширением pgvector
...
Команда запускает контейнер `pgvector/pgvector:pg16` на `127.0.0.1:5432`. Отдельный Qdrant больше не нужен: локальная авторизация, каталог цен, поставщики и векторы каталога хранятся в PostgreSQL. Таблицы создаются backend автоматически при первой регистрации, логине, проверке токена, seed-admin запуске, загрузке каталога или поиске.

Без Makefile можно поднять только PostgreSQL:

```bash
docker compose up -d postgres
```

Или собрать и запустить весь стек в контейнерах через профиль `app`:

```bash
docker compose --profile app up --build
```

В этом режиме `frontend` доступен на `http://localhost:5173`, `backend` на `http://localhost:8000`, а backend подключается к PostgreSQL по Docker-сети через `postgres:5432`.
...
Откройте `http://localhost:5173`. Приложение сначала попросит войти или зарегистрироваться через локальную email/password авторизацию. После входа загрузите CSV-файл, например `data/prices.csv`, дождитесь статуса `ready` и используйте режимы `Планирование брифа`, `Семантический поиск` или вкладку `Каталог`.

После загрузки `data/prices.csv` backend:

- парсит строки каталога и использует готовую колонку `embedding` из CSV;
- сохраняет поставщиков в `catalog_suppliers`, услуги в `catalog_price_items`, векторы в `catalog_embeddings`;
- строит pgvector HNSW-индекс для семантического поиска;
- группирует строки по поставщикам для вкладки `Каталог`.
...
`make dev` запускает PostgreSQL/pgvector через Docker Compose, затем параллельно стартует FastAPI и Vite. Перед запуском убедитесь, что `.env` заполнен и зависимости установлены через `make install`. Остановить foreground-процессы можно через `Ctrl+C`, контейнер останавливается командой:
...
- Колонка `embedding` из CSV используется как готовый вектор каталога; backend не строит текст для catalog embeddings и эмбеддит только пользовательский запрос для поиска.
- Вкладка `Каталог` показывает поставщиков из PostgreSQL, объединяет строки по заказчику/поставщику и открывает список всех услуг выбранного поставщика.
...
- Режим `Планирование брифа` подбирает shortlist из нескольких подрядчиков под каждую потребность. Он не выбирает финального подрядчика за менеджера: итоговое сообщение собирает варианты, аргументы и вопросы для уточнения.
